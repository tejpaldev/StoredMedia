...
﻿gj.grid.private = {
    ...
    RowRenderer: function ($grid, $row, record, position) {
        ...
        $row.data("row", { id: id, record: null })
            ...
            switch (type) {
                case "date":
                    ...
                    if (dt.format && format) {
                        ...
                        text = month + "/" + day + "/" + dt.getFullYear() + "  " + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds() + ":" + dt.getMilliseconds();
                    }
                    break;
                case "money":
...
gj.grid.public = {
    ...
    render: function (response) {
        ...
        if (data) {
            ...
            var message = response[data.mapping.message];
            if (message != "" && message != undefined && records.length == 0) {
                $grid = this;
                $tbody = $grid.find("tbody");
                $tbody.find("tr[data-role='emptyRow']").remove();
                $tbody.empty(); 
                gj.grid.private.AppendEmptyRow(this, message);
